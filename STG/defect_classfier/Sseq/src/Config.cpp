...
std::unordered_map<std::string, bool> Config::getOptions(){
    std::unordered_map<std::string, bool>  opt;
    for (auto block : options) {
        for (auto blockOpt : block.second) {
            std::string opt_name = block.first+"_"+blockOpt.first;
            if (trim(blockOpt.second) == "false"){
                opt[opt_name]=false;
            }else{
                opt[opt_name]=true;
            }
