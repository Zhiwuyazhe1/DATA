...
// 计算是否是前缀/后缀，增加路径容错
static bool isSuffix(const std::string& str, const std::string& sub) {
    if (str.length() < sub.length()) {
        return false;
    }
    std::string str_sub = str.substr(str.length() - sub.length());
    return std::equal(str_sub.begin(), str_sub.end(), sub.begin());
}

static bool isPrefix(const std::string& str, const std::string& sub) {
    if (str.length() > sub.length()) {
        return false;
    }
    if (sub.rfind(str) == sub.size() - str.size()) {
        return true;
    } 
    return false;
}
...
#endif
    if(subprog){
        std::string file_name= subprog->getFile()->getFilename().str();
        return isSuffix(file_name,file) || isPrefix(file_name,file);
    }
    return false;
}
...
static bool fileMatch(const std::string &file, const llvm::GlobalVariable &G) {
    ...
    for (auto *GV : GVs) {
        ...
        std::string file_name= var->getFile()->getFilename().str();
        if (isSuffix(file_name,file) ){
            has_match = true;
            break;
        }
    }
...
static void getCriteriaInstructions(llvm::Module &M, LLVMPointerAnalysis *pta,
    ...
    if (parts.size() > 4 || parts.empty()) {
        llvm::errs() << parts.size()<< "WARNINGparts: ignoring invalid slicing criterion: "
                     << criterion << "\n";
        return;
    }
    ...
    if (!fun.empty() && obj.empty() && line == 0) {
        llvm::errs() << "WARNINGobj: ignoring invalid slicing criterion: "
                     << criterion << "\n";
        return;
    }
...
bool getSlicingCriteriaNodes(LLVMDependenceGraph &dg,
    ...
    std::string criteria ="";
    if (slicingCriteria != "") {
        auto slicingCriteriaParts = splitList(slicingCriteria, ',');//逗号分割多个切片准则
        for (auto &slicingCriteria : slicingCriteriaParts) {
            if (criteria != "")
                criteria += ";";
    ...
    auto crits =
            getSlicingCriteriaInstructions(*dg.getModule(), criteria,
...
static void getLineCriteriaNodes(LLVMDependenceGraph &dg,
    ...
    for (auto &crit : criteria) {
        ...
        if(find(parts[0].begin(),parts[0].end(),'#')!=parts[0].end()){ //add -lx
            auto fpart = splitList(parts[0],'#')[2];//fpart是filename # func # line 
            
            // parse the line number
            if (fpart.empty()) {
                // global variable
                parsedCrit.emplace_back(-1, parts[1]);
            } else if (isNumber(fpart)) {
                int line = atoi(fpart.c_str());
                if (line > 0)
...
getPrimarySlicingCriteriaNodes(LLVMDependenceGraph &dg,
    ...
    std::vector<std::string> line_criteria; // line_criteria都对应file#function#line
